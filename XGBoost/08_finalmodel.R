...
for(p in 1:5){
  ...
  if(shapley == "Y"){
    ...
    for (y in years_tobeselected) {
      ...
      shapleyvals_sub <- shap_values[idx, ]
...
if(standarderrors == "Y"){
  ...
  bootstrap_predictions <- function(model, X_data, n_bootstraps = 100) {
    ...
    for(i in 1:n_bootstraps) {
      ...
      # Create prediction using a subset of features and observations
      pred_params <- list(
        subsample = 0.8,
        colsample_bytree = 0.8,
        nrounds = model$niter * 0.9 # Slightly fewer rounds for diversity
      )
      
      # Make predictions with some randomness
      preds <- predict(model, X_data, iterationrange = c(1, model$niter * 0.9))
  ...
  for(p in 1:5) {
    ...
    lower_bounds <- apply(bootstrap_preds, 1, function(x) quantile(x, 0.025))
    upper_bounds <- apply(bootstrap_preds, 1, function(x) quantile(x, 0.975))
