#file 07_model_performance

# Define SVM tuning grid instead of elastic net
training_grid_modelperformance <- expand.grid(C = seq(0.1, 10, by = 0.5))
...
# This adds predictions based on the SVM model to the test data set
source("./scripts/07a_SVM_for_modelperformance.R")
...
fullmodel <- ggplot(test_data, aes(y=GDPpc, x=prediction_abs)) + stat_poly_eq(formula = y~x, data=test_data, aes(label = after_stat(rr.label)), parse = TRUE) +
  ...
  labs(x="prediction", y="log(GDP per capita)", title = "Full model (SVM)")
...
RMSE_tab_fullmodel$group <- "Full model (SVM)"
...
AME_tab_fullmodel$group <- "Full model (SVM)"
...
ggsave(paste0("./genfiles_", version, "/figures/Fig2_AB_SVM.svg"), width = 8, height = 4)
...
AME_distribution_full$group <- "Full model (SVM)"
...
R2_distribution_full$group <- "Full model (SVM)"
