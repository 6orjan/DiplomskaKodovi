...
# Prepare the training data and test data for Random Forrest
train_matrix <- as.matrix(training_data[, -c("GDPpc", "prediction_abs")])
test_matrix <- as.matrix(test_data[, -c("GDPpc", "prediction_abs")])
...
# Make predictions
test_data$prediction <- predict(rf_model, newdata = test_matrix)
...
for(iteration in 1:n_draws){
  ...
  tryCatch(
    expr= {
      ...
      # This adds predictions based on the Random Forrest model to the test data set
      source("./scripts/07a_Rf_for_modelperformance.R")
